---
title: "NASA Battery - Team Project"
...
ensure_numeric <- function(data, columns) {
  for (col in columns) {
...
discharge29 <- ensure_numeric(discharge29, c('Current_measured', 'Voltage_measured'))
discharge38 <- ensure_numeric(discharge38, c('Current_measured', 'Voltage_measured'))
discharge25 <- ensure_numeric(discharge25, c('Current_measured', 'Voltage_measured'))
discharge33 <- ensure_numeric(discharge33, c('Current_measured', 'Voltage_measured'))
discharge36 <- ensure_numeric(discharge36, c('Current_measured', 'Voltage_measured'))
discharge41 <- ensure_numeric(discharge41, c('Current_measured', 'Voltage_measured'))
...
create_plot <- function(data, x_range = NULL, hline, title) {
  ...
  ggplot(data) +
    geom_line(aes(x = seq_along(Current_measured), y = Current_measured, color = 'Current')) +
    geom_line(aes(x = seq_along(Voltage_measured), y = Voltage_measured, color = 'Voltage')) +
...
# Create each plot
p1 <- create_plot(discharge29[3801:4400, ], hline = 2.0, title = 'CASE A | One Cycle')
p2 <- create_plot(discharge38, hline = 2.2, title = 'CASE B | Whole Cycle')
p3 <- create_plot(discharge25[3911:4251, ], hline = 2.0, title = 'CASE C | One Cycle')
p4 <- create_plot(discharge33[4073:4152, ], hline = 2.0, title = 'CASE D | One Cycle')
p5 <- create_plot(discharge36[4073:4410, ], hline = 2.7, title = 'CASE E | One Cycle')
p6 <- create_plot(discharge41, hline = 2.0, title = 'CASE F | One Cycle')


ggplot(discharge29[3801:4400, ]) +
  geom_line(aes(x = seq_along(Current_measured), y = Current_measured, color = 'Current')) +
  geom_line(aes(x = seq_along(Voltage_measured), y = Voltage_measured, color = 'Voltage')) +
