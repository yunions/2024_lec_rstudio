...
y <- test1pca$Stage1_Output
Model1 <- predict(mlppca[[1]], newdata=test1pca)
Model2 <- predict(svr[[1]], newdata=testsc1)
Model3 <- predict(svrpca[[1]], newdata=test1pca)

df_pred <- data.frame(Index = 1:4185, Original = y, MLPpca = Model1, SVR = Model2, SVRpca = Model3)

df_long <- melt(df_pred, id.vars = "Index")

# 플롯 생성
ggplot(df_long, aes(x = Index, y = value, color = variable)) +
  geom_line() +
  labs(title = "Comparison of Models with Original y",
       x = "Index",
       y = "Value",
       color = "Legend") +
  theme_minimal()

y <- test2pca$Stage2_Output
Model1 <- predict(mlppca[[2]], newdata=test2pca)
Model2 <- predict(svr[[2]], newdata=testsc2)
Model3 <- predict(svrpca[[2]], newdata=test2pca)
