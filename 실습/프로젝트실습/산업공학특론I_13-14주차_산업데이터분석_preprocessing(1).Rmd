...
pca <- prcomp(trainsc1 [,], center = TRUE, scale. = TRUE)

stage1_PCA <- pca$x[, 1:3]
...
model1 <- mlp1$finalModel
model2 <- mlp2$finalModel
test1 <- testsc1
test2 <- testsc2

evaluate <- function(model1, model2, test1, test2){
  
  pred1 <- predict(model1, newdata=test1)
  pred2 <- predict(model2, newdata=test2)
  
  print("Stage1")
  result <- c(mae(pred1, testsc1$Stage1_Output),
  ...
  print("Stage2")
  result <- c(mae(pred2, testsc2$Stage2_Output),
...
evaluate(mlp1$finalModel, mlp2$finalModel, testsc1, testsc2)
...
dt1 <- train(Stage1_Output ~ ., data=trainsc1, method="rpart",
            trControl=train_control, control=rpart_control,
            tuneGrid=dt_grid)

using1 <- predict(dt1$finalModel, newdata=trainsc1)
trainsc2$stage1_ouput_exp <- using1
dt2 <- train(Stage2_Output ~ ., data=trainsc2, method="rpart",
            trControl=train_control, control=rpart_control,
            tuneGrid=dt_grid)
...
svr1 <- train(Stage1_Output ~ ., data=trainsc1, method="svmRadial",
            trControl=train_control, tuneGrid=svm_grid)
using1 <- predict(svr1$finalModel, newdata=trainsc1)
trainsc2$stage1_ouput_exp <- using1
svr2 <- train(Stage2_Output ~ ., data=trainsc2, method="svmRadial",
            trControl=train_control, tuneGrid=svm_grid)
...
mlp1 <- train(Stage1_Output ~ ., data=trainsc1, method="nnet",
            trControl=train_control, tuneGrid=mlp_gird,
            linout = T, trace = T)
using1 <- predict(mlp1$finalModel, newdata=trainsc1)
trainsc2$stage1_ouput_exp <- using1
mlp2 <- train(Stage2_Output ~ ., data=trainsc2, method="nnet",
            trControl=train_control, tuneGrid=mlp_gird,
            linout = T, trace = T)
print("MLP")
using1 <- predict(mlp1$finalModel, newdata=testsc1)
testsc2$stage1_ouput_exp <- using1
evaluate(mlp1$finalModel, mlp2$finalModel, testsc1, testsc2)
